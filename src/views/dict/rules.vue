        ...
        >
          <template #action="{ record }">
            <a-space>
              ...
              <a-button type="primary" size="small" danger @click="handleDel(record.name)">删除</a-button>
            </a-space>
          </template>
        </ym-table>
      </template>
    </CommonMangerPage>
    <NewRule ref="detailInstance" :getSourceData="getSourceData" :handleRefresh="handleRefresh" />
  </div>
</template>
...
import NewRule from '@/components/dict/NewRule.vue'
import { apiGetRuleList, apiRuleStatus } from '@/service/api/dict'
import { rulesColumns } from '@/columns/dict'
...
const handleDel = (name: string) => {
  YmModal.delete(async () => {
    const { code } = await apiRuleStatus({ name, status: 3 })
    if (code === 20001) {
...
const handleAddressStatus = async (record: any) => {
  const { code } = await apiRuleStatus({ name: record.name, status: record.status === '启用' ? 2 : 1 })
  if (code === 20001) {
